...
{
    public class SqliteDataBaseFake : IDataBase
    {
        private SqliteConnection myConnection;
        private Mutex mutex;
        public SqliteDataBaseFake()
        {
            mutex = new Mutex();
        ...
        {      
            ...
            deleteFlights();
           /* // Add flights with the same id.
            addFlightPlansForFake(80, 80, 250, 8, 4, "Flight with Same Id", true, true);
            addFlightPlansForFake(12, 3, 100, 6, 7, "FlightSame Id2", true, true);
            if (isGoodTest(1, 2))
            {
                Console.WriteLine("The test of flight with same id was successful");
            }
            else
            {
                Console.WriteLine("There is a problem with test of same id");
            }*/
            // Add 2 flight plans.
            ...
            //Thread.Sleep(3000);
            DeleteFlightPlanFromTable(idDelete);
            if (isGoodTest(7, 8))
            {
                // print good test all.
        ...
        private void deleteFlights()
        {
            ...
            flightsNow = GetFlightsByDateTime(date);
            if (flightsNow == null)
            {
                return;
            }
        ...
        private string addFlightPlansForFake(int longitudeStart, int latitudeStart, int passangers, int inceaseLong, int increaseLat, string companyName, bool sameId, bool withSegments)
        {
            ...
            if (sameId)
            {
                id = this.AddFlightPlanForFakeWithSameId(flightPlanFake1);
            }
            else
            {
                id = this.AddFlightPlan(flightPlanFake1);
            }
            return id;
        }

        public string AddFlightPlanForFakeWithSameId(FlightPlan flightPlan)
        {
            if (flightPlan.Location == null || flightPlan.Segments == null) { return null; }
            OpenConnection();
            // Random id.
            string id = "AA11111AAA";
